...
def check_path(flie_path: str, is_file: bool = False, auto_create: bool = False):
    if flie_path is not None:
        abspath = os.path.abspath(flie_path)
        if os.path.exists(abspath):
            if is_file and not os.path.isfile(abspath):
                logger.warning("path {} must be a file".format(flie_path))
                exit(0)
            if not is_file and not os.path.isdir(abspath):
                logger.warning("path {} must be a folder".format(flie_path))
                exit(0)
        else:
            if not auto_create:
                logger.warning("path doesn't exist: {} ".format(flie_path))
                exit(0)
            else:
                if is_file:
                    logger.info("file automatically created: {}".format(flie_path))
                    ...
                    logger.info("directory automatically created: {}".format(flie_path))
...
group.add_argument(
    "--dir", default=None, help="批量辅种的文件夹 folder for batch cross-seeding"
)
group.add_argument(
    "--single-dir", default=None, help="单个辅种的文件夹 folder for just one cross-seeding"
)
group.add_argument(
    "--torrent-dir",
    default=None,
    help="存储待辅种的种子的文件夹 folder containing .torrent files for cross-seeding",
)
group.add_argument(
    "--single-torrent", default=None, help="单个辅种的种子文件 .torrent file for cross-seeding"
)
parser.add_argument(
    "--site",
    required=True,
    choices=common.SITE_CONST.keys(),
    help="扫描的站点 the tracker to scan for cross-seeding.",
)
parser.add_argument(
    "--result-dir", required=True, help="储存扫描结果的文件夹 folder for saving scanned results"
)
parser.add_argument(
    "--api-frequency",
    default=None,
...
result_url_undownloaded_path = os.path.join(
    args.result_dir, "result_url_undownloaded.txt"
)
...
def scan(*, flist: list, tsize: int, scan_source: str, api):
    ...
    for file_name in sorted(flist, key=lambda f_name: -len(f_name))[:5]:
        resp = api.search_torrent_by_filename(file_name, use_cache=False)
        ...
        torrents = sum(
            [
                group["torrents"]
                for group in resp["response"]["results"]
                if "torrents" in group.keys()
            ],
            [],
        )
        for t in torrents:
        ...
        if not args.no_download:
            try:
                ...
                if GLOBAL["cnt_dl_fail"] <= 10:
                    ...
                    logger.info(
                        "下载种子(id {})失败，这可能是因为触发了{}对于非浏览器下载该种子的限制。"
                    ...
                    if GLOBAL["cnt_dl_fail"] == 10:
                        logger.info(
                            "suppress further hinting for .torrent file downloading failure"
                        )
...
if args.single_dir:
    ...
    logger.info("scanning {} for cross seeding in {}".format(folder, args.site))
    handle_folder(folder, api)
elif args.single_torrent:
    t_path = os.path.abspath(args.single_torrent)
    logger.info(
        "scanning {} for cross seeding in {}".format(args.single_torrent, args.site)
    )
    handle_torrent(t_path, api)
elif args.dir:
    try:
        ...
        logger.info(
            "{}/{} unscanned folders found in {}, start scanning for cross-seeding {}".format(
        ...
        logger.info(
            "{} folders scanned, {} torrents found, {} .torrent files downloaded from {}".format(
    ...
    try:
        ...
        logger.info(
            "{}/{} unscanned torrents found in {}, start scanning for cross-seeding {}".format(
                len(unscanned_torrents), len(candidates), args.torrent_dir, args.site
            )
        )
        for i, t_path in enumerate(unscanned_torrents):
            logger.info("{}/{} {}".format(i + 1, len(unscanned_torrents), t_path))
            handle_torrent(t_path, api)
    except Exception as e:
        print(e)
        logger.info(traceback.format_exc())
        ...
        logger.info(
            "{} torrents scanned, {} torrents found, {} .torrent files downloaded from {}".format(
                GLOBAL["scanned"], GLOBAL["found"], GLOBAL["downloaded"], args.site
            )
        )
